...
WORKDIR /src
COPY ["BookShare.csproj", "."]
RUN dotnet restore "./BookShare.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./BookShare.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./BookShare.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
